...
if numofunknown>0
    unknownmat=cell(1,numofunknown);
    for o=1:numofunknown
        unknownmatholder=uigetdir('Z:\Co-workers\Technologists\Brian\Data\Assays\2011-09-23\','Please Select Source Folder fot the unknown');
            unknownmat(o)={unknownmatholder};
    end
    [uklp uklpsd uklpcv] = zerounknown(hms,unknownmat,exposures,2,numofmethods,methodvalue);
else
    uklp=[];
    uklpsd=[];
    uklpcv=[];
...
for g=1:numel(ukval)
    uk=ukval{g};
    unknownval=uk(sexpval);
    realukholder(g)=unknownval;
end
realukholder'
end
